...
export function Header({
  ...
  const statusMap = {
    ...
    tutor: "Senior Mathematics Lecturer",
  ...
  return (
    <header className="fixed top-0 right-0 left-0 h-16 bg-white border-b border-brand-border/40 z-30 flex items-center justify-between px-6 select-none font-sans">
      ...
      <div className="flex items-center gap-4">
        {/* Advanced Interactive Role Switch Panel */}
        <div className="flex items-center bg-brand-low p-1 rounded-xl border border-brand-border/20 self-center">
          <button
            onClick={() => onChangeRole("student")}
            className={`px-3 py-1.5 rounded-lg text-xs font-display font-bold tracking-wider transition-all duration-300 ${
              activeRole === "student"
                ? "bg-brand-primary text-white shadow-md shadow-brand-primary/15"
                : "text-brand-text-variant hover:bg-brand-container/60 hover:text-brand-text"
            }`}
          >
            STUDENT
          </button>
          <button
            onClick={() => onChangeRole("tutor")}
            className={`px-3 py-1.5 rounded-lg text-xs font-display font-bold tracking-wider transition-all duration-300 ${
              activeRole === "tutor"
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-display font-bold tracking-wider transition-all duration-300 ${
              activeRole === "admin"
                ? "bg-brand-text text-white shadow-md shadow-brand-text/15"
                : "text-brand-text-variant hover:bg-brand-container/60 hover:text-brand-text"
            }`}
          >
            ADMIN
