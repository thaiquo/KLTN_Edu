...
export default function App() {
  ...
  const [profileSettings, setProfileSettings] = useState<AppProfileSettings>(INITIAL_PROFILE_SETTINGS);
  ...
  const renderMainContent = () => {
    switch (currentPage) {
      ...
      case "tutor-approval":
        // Admin Onboarding Tutor Approval sub-panel
        const pendingTutorAudits = users.filter((u) => u.role === "Tutor" && u.status === "Pending");
        return (
          <div className="space-y-6 select-none font-sans max-w-5xl mx-auto pb-10">
            <div>
              <h2 className="font-display font-black text-xl lg:text-2xl text-brand-text">
                Academic Onboarding approvals
              </h2>
              <p className="text-brand-text-variant/60 text-xs mt-1">
                Verify credential AWS/IELTS score certifications of tutor applications before publishing profiles.
              </p>
            </div>
              ...
              <table className="w-full text-left">
                <thead className="bg-brand-low text-brand-text-variant/60 font-display text-[10px] font-bold uppercase tracking-wider">
                  <tr>
                    ...
                    <th className="px-6 py-4">Documentation Verification</th>
                    <th className="px-6 py-4 text-right">Acquisition</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-brand-border/10 text-xs font-semibold text-brand-text">
                  {pendingTutorAudits.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="p-12 text-center text-brand-text-variant/60">
                        <Sparkles className="w-8 h-8 text-yellow-400 mx-auto mb-2" />
                        <p className="font-bold">All onboarding tutor files reviewed!</p>
                      </td>
                    </tr>
                  ) : (
                    pendingTutorAudits.map((onb) => (
                      <tr key={onb.id} className="hover:bg-brand-low/30 transition-colors">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <img className="w-9 h-9 rounded-full object-cover" src={onb.avatarUrl} alt={onb.name} referrerPolicy="no-referrer" />
                            <div>
                              <p className="font-bold">{onb.name}</p>
                              <p className="text-xs text-brand-text-variant/60">{onb.email}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className="px-2.5 py-0.5 bg-amber-50 text-amber-700 border border-amber-200 text-[10px] font-bold uppercase rounded-full tracking-wider">
                            {onb.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-brand-text-variant">
                          <span className="flex items-center gap-1">
                            <FileText className="w-4 h-4 text-brand-primary" />
                            Applied_Tutor_Credentials_Bundle.pdf
                          </span>
                        </td>
                        <td className="px-6 py-4 text-right">
                          <button
                            onClick={() => {
                              alert(`Tutor verification granted for ${onb.name}! Their workspace is now fully legal and active.`);
                              setUsers((prev) =>
                                prev.map((u) => (u.id === onb.id ? { ...u, status: "Active" } : u))
                              );
                            }}
                            className="px-3.5 py-1.5 bg-brand-primary text-white hover:bg-brand-primary/95 text-[10px] font-display font-black tracking-widest rounded-xl transition-all shadow-sm cursor-pointer"
                          >
                            VERIFY APPROVAL
                          </button>
                        </td>
