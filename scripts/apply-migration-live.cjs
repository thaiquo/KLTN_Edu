// Applies canonical migrations to live kltn-postgres / kltn_db safely with pre & post verification.
const fs = require('node:fs');
const path = require('node:path');
const { spawnSync } = require('node:child_process');
const root = path.resolve(__dirname, '..');

const container = 'kltn-postgres';
...
console.log(`Verifying pre-migration state on ${container}/${database}...`);
const preCheck = {
  users: Number(sql("SELECT count(*) FROM users;")),
  classRooms: Number(sql("SELECT count(*) FROM class_rooms;")),
  classSessions: Number(sql("SELECT count(*) FROM class_sessions;")),
  attendances: Number(sql("SELECT count(*) FROM session_attendances;")),
  agreements: Number(sql("SELECT count(*) FROM contract_agreement;")),
};
console.log('Pre-migration row counts:', preCheck);
...
for (const service of ['account', 'learning', 'contract', 'notification']) {
  ...
  if (legacyLearning) {
    ...
    statements.push('CREATE SCHEMA IF NOT EXISTS integration_archive;',
      `CREATE TABLE IF NOT EXISTS integration_archive.${table}_before AS TABLE public.${table};`);
  ...
  console.log(`${service}: verified canonical migration history; ${pending.length} pending scripts applied`);
}

console.log(`Verifying post-migration state on ${container}/${database}...`);
const postCheck = {
  users: Number(sql("SELECT count(*) FROM users;")),
  classRooms: Number(sql("SELECT count(*) FROM class_rooms;")),
  classSessions: Number(sql("SELECT count(*) FROM class_sessions;")),
...
console.log('Post-migration row counts:', postCheck);

for (const key of Object.keys(preCheck)) {
  if (preCheck[key] !== postCheck[key]) {
    throw new Error(`CRITICAL: Row count mismatch for ${key}! Pre: ${preCheck[key]}, Post: ${postCheck[key]}`);
  }
}

console.log('All business data preserved perfectly. Flyway schema history is 100% synchronized!');
