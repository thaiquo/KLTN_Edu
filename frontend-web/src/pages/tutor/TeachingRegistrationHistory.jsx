...
export function TeachingRegistrationHistory({ registrations, suggestions = [] }) {
  ...
  return <>
    <section className="rounded-[8px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      {registrations.length > 0 && <div className="mt-5 grid gap-3">{registrations.map((item) => <RegistrationSummary key={item.id} item={item} onOpen={() => setSelected(item)} />)}</div>}

      {suggestions.length > 0 && <div className="mt-6 border-t border-slate-200 pt-5"><p className="text-xs font-black uppercase tracking-wider text-slate-500">Môn mới đang đề xuất</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{suggestions.map((item) => <article key={item.id} className="rounded-[8px] border border-slate-200 bg-slate-50 p-4"><p className="font-extrabold text-slate-950">{item.subjectName} · {item.levelName}</p><p className="mt-2 text-xs font-bold text-slate-500">{item.status === 'APPROVED' ? 'Đã thêm vào danh mục' : item.status === 'REJECTED' ? 'Không được chấp nhận' : 'Admin đang xem xét'}</p></article>)}</div></div>}
    </section>
