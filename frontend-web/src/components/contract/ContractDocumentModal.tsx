...
import { Building, Check, CheckCircle2, Copy, Download, FileSignature, FileText, GraduationCap, Printer, RefreshCw, User, X } from "lucide-react";
...
function SignatureCard({ title, party, proof, copiedField, onCopy }: {
  title: string;
...
export function ContractDocumentModal({ agreementId, onClose }: ContractDocumentModalProps) {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      setDocument(await contractsApi.getContractDocument(agreementId));
    } catch (loadError) {
      console.error("Failed to load contract document:", loadError);
      setDocument(null);
      setError("Không thể tải dữ liệu hợp đồng từ hệ thống. Vui lòng thử lại.");
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => { void loadData(); }, [agreementId]);
      ...
      <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[94vh] print:max-h-none print:shadow-none print:border-none print:rounded-none">
        <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between print:hidden">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-blue-600/30 border border-blue-500/40 flex items-center justify-center text-blue-400"><FileSignature className="w-5 h-5" /></div>
            <div>
              <h2 className="font-display font-black text-sm sm:text-base tracking-tight">Văn Bản Hợp Đồng Điện Tử EIP-712 & Ký Quỹ Escrow</h2>
              <p className="text-[11px] text-slate-400 font-semibold">Dữ liệu hợp đồng #{agreementId.slice(0, 8)} từ Contract Service</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="p-2 text-slate-400 hover:text-white rounded-xl hover:bg-slate-800 transition-colors"><X className="w-5 h-5" /></button>
          </div>
        </div>
        ...
        <div className="p-6 sm:p-10 overflow-y-auto space-y-6 text-slate-900 leading-relaxed font-serif" id="printable-contract-document">
          {loading ? (
            <div className="py-20 flex flex-col items-center justify-center gap-3 text-slate-400 font-sans"><RefreshCw className="w-8 h-8 animate-spin text-blue-600" /><span className="text-xs font-bold">Đang tải dữ liệu hợp đồng...</span></div>
          ) : error || !document ? (
            <div className="py-16 flex flex-col items-center justify-center gap-4 text-center font-sans">
              ...
              <button onClick={() => void loadData()} className="px-4 py-2 rounded-xl bg-blue-600 text-white text-xs font-bold">Tải lại</button>
            </div>
          ) : (
            <>
              <div className="text-center space-y-1 border-b-2 border-slate-900 pb-4">
                ...
                <h1 className="font-display text-base sm:text-xl font-black pt-2 tracking-tight uppercase">HỢP ĐỒNG DỊCH VỤ KẾT NỐI GIA SƯ VÀ HỌC TẬP TRỰC TUYẾN</h1>
                <p className="text-sm font-sans font-bold text-blue-800 pt-1">Khóa học: {displayValue(document.className)}</p>
                <p className="text-[11px] font-mono text-slate-600">Mã hợp đồng: <span className="font-bold text-slate-900">{document.agreementId}</span></p>
                <p className="text-[11px] font-mono text-slate-500 truncate max-w-xl mx-auto">Hash tham chiếu On-chain: {displayValue(document.onchainAgreementId)}</p>
              </div>

              <div className="space-y-1 text-xs text-slate-700 italic border-b border-slate-200 pb-4">
                <p>• Căn cứ Bộ luật Dân sự số 91/2015/QH13;</p><p>• Căn cứ Luật Giao dịch điện tử số 20/2023/QH15;</p><p>• Căn cứ nhu cầu học tập và thỏa thuận của các bên trên nền tảng EduConnect.</p>
                <p className="not-italic pt-1 font-semibold text-slate-800">Hợp đồng được khởi tạo ngày <strong>{formatDate(document.createdAt)}</strong>, gồm các bên:</p>
              </div>
              ...
              <div className="space-y-4 font-sans text-xs">
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
                  <p className="font-extrabold text-emerald-900 uppercase text-xs flex items-center gap-1.5 border-b border-slate-200 pb-1.5"><Building className="w-4 h-4 text-emerald-600" /> 3. BÊN TRUNG GIAN (BÊN C - EDUCONNECT SMART ESCROW)</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-slate-800 pt-1">
                    <p>Mạng Blockchain: <strong>{blockchainNetwork(document.platform.chainId)}</strong></p><p>Token: <strong>{displayValue(document.financialTerms.tokenSymbol)}</strong></p>
                    <p className="sm:col-span-2 font-mono text-[11px] text-slate-600 truncate">Ví nền tảng: <strong>{displayValue(document.platform.walletAddress)}</strong></p>
                    <p className="sm:col-span-2 font-mono text-[11px] text-slate-600 truncate">Smart Contract Escrow: <strong>{displayValue(document.platform.escrowContractAddress)}</strong></p>
                  </div>
                </div>
              </div>

              <div className="space-y-4 pt-2 text-xs sm:text-[13px] text-slate-800 leading-relaxed">
                <Clause title="ĐIỀU 1: ĐỐI TƯỢNG HỌC TẬP VÀ PHẠM VI GIẢNG DẠY">
                  <p>1.1. Bên A nhận cung cấp dịch vụ giảng dạy cho lớp <strong>{displayValue(document.className)}</strong>.</p>
                  <p>1.2. Tổng số buổi học theo hợp đồng là <strong>{document.financialTerms.totalSessions} buổi</strong>.</p>
                  <p>1.3. Hình thức học tập: Trực tuyến (Online) trên nền tảng giáo dục EduConnect.</p>
                </Clause>
                <Clause title="ĐIỀU 2: HỌC PHÍ, BẢO CHỨNG ESCROW VÀ LỊCH GIẢI NGÂN">
                  <p>2.1. Học phí mỗi buổi: <strong>{formatDecimal(document.financialTerms.pricePerSessionUsdc)} {document.financialTerms.tokenSymbol}</strong>{document.financialTerms.pricePerSessionVnd && <> ({formatDecimal(document.financialTerms.pricePerSessionVnd, 0)} VNĐ)</>}.</p>
                  <p>2.2. Tổng giá trị hợp đồng: <strong>{formatDecimal(document.financialTerms.totalAmountUsdc)} {document.financialTerms.tokenSymbol}</strong>{document.financialTerms.totalPriceVnd && <> ({formatDecimal(document.financialTerms.totalPriceVnd, 0)} VNĐ)</>}.</p>
                  <p>2.3. Tỷ giá quy đổi cố định: <strong>1 USDC = {formatDecimal(document.financialTerms.vndPerUsdc, 0)} VNĐ</strong>.</p>
                  <p>2.4. Quy tắc giải ngân: 100% học phí được khóa trong Smart Contract Escrow của Bên C. Sau mỗi buổi học điểm danh thành công, Smart Contract tự động giải ngân 85% cho Bên A và 15% phí dịch vụ cho Bên C.</p>
                </Clause>
              ...
              <div className="pt-6 border-t-2 border-slate-900 space-y-4 font-sans">
                <h3 className="font-black text-slate-950 uppercase text-xs tracking-wider text-center">XÁC NHẬN CHỮ KÝ ĐIỆN TỬ (EIP-712 CRYPTOGRAPHIC PROOF)</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <SignatureCard title="CHỮ KÝ BÊN A (GIA SƯ)" party={document.tutor} proof={document.tutorSignature} copiedField={copiedField} onCopy={copyToClipboard} />
                  <SignatureCard title="CHỮ KÝ BÊN B (HỌC VIÊN)" party={document.student} proof={document.studentSignature} copiedField={copiedField} onCopy={copyToClipboard} />
                </div>
                <div className="p-3 bg-slate-100 rounded-xl text-[11px] font-mono text-slate-500 space-y-1">
                  <p className="break-all">Terms Hash: <strong className="text-slate-700">{document.termsHash}</strong></p><p>Phiên bản hợp đồng: <strong>{document.contractVersion}</strong></p>
                  <p className="text-[10px] text-slate-400 font-sans italic">Dữ liệu được đọc từ contract_agreement và contract_acceptance của đúng mã hợp đồng; trường còn thiếu hiển thị “Chưa cập nhật”.</p>
                </div>
              </div>
            </>
          )}
...
function PartySection({ kind, party }: { kind: "tutor" | "student"; party: ContractDocumentParty }) {
  ...
  return (
    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-slate-800 pt-1">
        <p>Họ và tên: <strong className="text-slate-950 text-sm font-black">{displayValue(party.fullName)}</strong></p><p>Email: <strong>{displayValue(party.email)}</strong></p><p>Số điện thoại: <strong>{displayValue(party.phone)}</strong></p>
