...
export function ContractAuditTimeline({
  ...
  const [outcomeInput, setOutcomeInput] = useState<'BOTH_PRESENT' | 'STUDENT_ABSENT_TUTOR_PRESENT' | 'TUTOR_ABSENT'>('BOTH_PRESENT');
  ...
  const handleProposeSettlement = async () => {
    ...
    try {
      ...
      const result = await contractsApi.proposeSettlement(agreementId, {
        sessionId: Number(sessionIdInput),
        outcome: outcomeInput,
        evidence: evidenceInput,
      });
      setActionMessage(`Proposal queued: ${result.transactionStatus}`);
      await loadAuditData();
    } catch (err: any) {
      setActionMessage(err?.message || 'Cannot propose session settlement.');
    } finally {
      setActionBusy(false);
    }
  };
  ...
  const handleFinalizeSettlement = async (settlementId: string) => {
    try {
      setActionBusy(true);
      setActionMessage(null);
      const result = await contractsApi.finalizeSettlement(settlementId);
      setActionMessage(`Finalization queued: ${result.transactionStatus}`);
      await loadAuditData();
    } catch (err: any) {
      setActionMessage(err?.message || 'Cannot finalize session settlement.');
    } finally {
      setActionBusy(false);
    }
  };
        ...
        <div className="mb-6 rounded-2xl border border-slate-200 bg-slate-50/80 p-4 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <input
              ...
              className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-blue-100"
              placeholder="Session"
            />
            <select
              value={outcomeInput}
              onChange={(event) => setOutcomeInput(event.target.value as typeof outcomeInput)}
              className="sm:col-span-2 rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-blue-100"
            >
              <option value="BOTH_PRESENT">BOTH_PRESENT</option>
              <option value="STUDENT_ABSENT_TUTOR_PRESENT">STUDENT_ABSENT_TUTOR_PRESENT</option>
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-blue-100"
            placeholder="Evidence note or reference"
          />
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {canFinalize && (
                        <button
                          ...
                          disabled={actionBusy}
                          className="rounded-lg bg-emerald-600 px-3 py-1.5 font-black text-white disabled:opacity-50"
                        >
                          Finalize
                        </button>
                      )}
