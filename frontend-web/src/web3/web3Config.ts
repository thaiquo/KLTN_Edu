...
export const CONTRACT_ADDRESSES: Record<number, { escrow: string; usdc: string }> = {
  31337: {
    escrow: import.meta.env.VITE_ESCROW_CONTRACT_ADDRESS || '0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512',
...
export const ESCROW_ABI = [
  ...
  'function getSessionSettlement(bytes32 agreementId, bytes32 sessionId) external view returns (tuple(uint8 proposedOutcome, uint8 status, uint64 disputeDeadline, bytes32 proposalEvidenceHash, bytes32 disputeEvidenceHash, bytes32 resolutionHash))',
  ...
  'event SessionSettlementProposed(bytes32 indexed agreementId, bytes32 indexed sessionId, uint8 outcome, uint64 disputeDeadline, bytes32 evidenceHash)',
  'event SessionSettled(bytes32 indexed agreementId, bytes32 indexed sessionId, uint8 outcome, uint8 finalStatus, uint256 tutorAmount, uint256 platformAmount, uint256 studentRefund)',
