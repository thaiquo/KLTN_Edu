...
class EnrollmentRequestServiceNotificationTest {
    ...
    void enrollClassPublishesEnrollmentRequestedToTutor() {
        ...
        when(classRoomRepository.findByIdForUpdate(10L))
                .thenReturn(Optional.of(classRoom));

        when(enrollmentRequestRepository.countByClassRoomIdAndStatus(
                10L,
                EnrollmentRequestStatus.ACCEPTED)).thenReturn(0L);
        ...
        when(enrollmentRequestRepository.countByClassRoomIdAndStatus(
                10L,
                EnrollmentRequestStatus.PENDING)).thenReturn(0L);

        when(tutorAuthorizationStateRepository.findByTutorProfileId(55L))
                .thenReturn(Optional.of(tutorState(200L)));
    ...
    void acceptRequestPublishesEnrollmentAcceptedToStudent() {
        ...
        when(enrollmentRequestRepository.findById(500L))
        ...
        when(enrollmentRequestRepository.save(request))
                .thenReturn(request);
        ...
        verify(eventPublisher).publishEnrollmentAccepted(
                500L,
                10L,
                100L,
                200L,
    ...
    void cancelRequestPublishesEnrollmentCancelledToTutor() {
        ...
        verify(eventPublisher).publishEnrollmentCancelled(
                500L,
                10L,
                200L,
                100L,
