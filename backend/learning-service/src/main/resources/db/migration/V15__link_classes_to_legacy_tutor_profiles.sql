...
-- In the current service-based architecture, tutors/users may live outside
-- the learning-service database. Skip this migration when legacy tables
-- are not present.
...
BEGIN
    IF to_regclass('public.tutors') IS NOT NULL
       AND to_regclass('public.users') IS NOT NULL THEN
        ...
        FROM tutors tutor
        JOIN users account_user
            ON account_user.id = tutor.user_id
