...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (SecurityContextHolder.getContext().getAuthentication() == null) {
            try {
                ...
                // Access token hết hạn/không hợp lệ: tiếp tục như chưa xác thực để trả 401,
                // frontend sẽ dùng refresh token để cấp lại phiên.
    ...
    private UsernamePasswordAuthenticationToken buildAuthentication(Account account, HttpServletRequest request) {
        AuthPrincipal principal = new AuthPrincipal(account.getId(), account.getEmail(), account.getRoles(), account.getTokenVersion());
