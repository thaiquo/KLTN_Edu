...
class AuthFlowIntegrationTest {
    ...
    @Test
    void newlyRegisteredAccount_shouldBeUsableWithIssuedLoginCookies() throws Exception {
        ...
        String email = "pending-student@example.com";
    ...
    void register_verify_login_refresh_me_logout_flow_shouldWork() throws Exception {
        ...
        assertThat(account.getStatus()).isEqualTo(iuh.fit.account_service.shared.enums.AccountStatus.PENDING_VERIFICATION);
        assertThat(account.getRoles()).containsExactly(iuh.fit.account_service.shared.enums.Role.STUDENT);
        ...
        assertThat(account.getStatus()).isEqualTo(iuh.fit.account_service.shared.enums.AccountStatus.ACTIVE);
    ...
    @Test
    void studentWithTutorRole_shouldStillLoginInStudentMode() throws Exception {
        doNothing().when(loginRateLimitService).ensureAllowed(anyString(), anyString());
        doNothing().when(loginRateLimitService).reset(anyString(), anyString());

        String email = "student-tutor@example.com";
        String password = "Password123!";
        mockMvc.perform(post("/auth/register")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
                    {
                      "fullName": "Student Tutor",
                      "email": "%s",
                      "phone": "0900000003",
                      "password": "%s",
                      "confirmPassword": "%s"
        ...
        account.assignRole(tutorRole, null);
        ...
        mockMvc.perform(post("/auth/login")
            ...
            .andExpect(jsonPath("$.data.user.roles.length()").value(2));
    }
