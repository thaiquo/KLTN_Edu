...
public class TerminationService {
    ...
    public record View(TerminationCase request, List<ItemView> items) {}
    private View view(TerminationCase c) {
        return new View(c, items.findByCaseIdOrderByAgreementId(c.getId()).stream().map(i -> {
    ...
    public View request(UUID agreementId, boolean wholeClass, String reason, String signature, String signerWallet, Long requestedAtTimestamp, ContractUserPrincipal user) {
        ...
        if (isStudent || isTutor) {
            ...
            long requestedAt = requestedAtTimestamp != null ? requestedAtTimestamp : System.currentTimeMillis() / 1000L;
        ...
        c.setStatus("REQUESTED"); c.setCreatedAt(OffsetDateTime.now()); c.setAuditJson("[]");
        ...
        switch (action) {
            ...
            case "APPROVE" -> {
                ...
                for (var target : targets) {
                    ...
                    locked.setTerminationCutoffSession(-1);
    ...
    public void requireClassCanCreate(Long classroomId) {
        ...
        if (cases.findByClassroomIdOrderByCreatedAtDesc(classroomId).stream()
                .anyMatch(c -> c.isWholeClass() && Set.of("APPROVED", "COMPLETED").contains(c.getStatus()))) {
